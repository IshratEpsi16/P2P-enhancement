...
export default function RfiDetailsForVatPage() {
  ...
  const sendRfi = async () => {
    if (validateRfi()) {
      ...
      try {
        ...
        console.log("prItemList: ", prItemList2);
      } catch (error) {
  ...
  let convertedPrItem: PrItemInterface[] = [];

  const updateLineItem = () => {
    if (prItemList2.length > 0) {
      convertedPrItem = prItemList2.map((e, i) => ({
    ...
    if (rfqIdInStore != null) {
      ...
      if (convertedPrItem.length !== 0) {
        for (let i = 0; i < convertedPrItem.length; i++) {
          console.log(rfqIdInStore);
          console.log(convertedPrItem[i]);
          console.log(convertedPrItem.length);

          updateLineItemToRfq(rfqIdInStore!, convertedPrItem[i]);
        }
        showSuccessToast("RFI done successfully");
      }
    }
  };
  ...
  const updateLineItemToRfq = async (
    rfqId: number,
    prItemList: PrItemInterface //SelectedPrItemInterface silo
  ) => {
    try {
      const result = await UpdateLineItemToRfqService(
        token!,
        rfqId,
        prItemList
      );
                ...
                {prItemList2.map((e, index) => {
                  return (
                    <tr key={index} className={`odd:bg-white even:bg-gray-50`}>
                      ...
                      <td className="px-6 py-4 text-[14px] text-blackColor font-mon whitespace-nowrap opacity-50 pointer-events-none">
                      ...
                      <td className="   px-6 py-4 text-[14px] text-blackColor font-mon  w-40 opacity-50 pointer-events-none">
                        <p>
                          {e.ITEM_DESCRIPTION === ""
                            ? "N/A"
                            : e.ITEM_DESCRIPTION}
        ...
        <Modal.Body>
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between">
              <div className="w-32">
                {rfiError.approveVal && (
                  <p className="text-red-500 text-xs font-mon w-32">
                    {rfiError.approveVal}
                  </p>
                )}
              </div>
