...
export default function RfiSupplierListPage() {
  ...
  //store
  const { setRfiSupplierListlength, setRfiTabNo, rfiTabNo } = useRfiStore();
  //store
  ...
  const getSupplierList = async () => {
    ...
    try {
      ...
      if (result.data.status === 200) {
        ...
        // if(result.data.data.OBJECT_TYPE === "Supplier Approval") {
        ...
        if (filteredData.length > 0) {
          ...
          setRegTotalData(filteredData.length);
        }

        const rfqFilteredData = result.data.data.filter(
          (item: RfiSupplierInterface) => item.OBJECT_TYPE === "RFQ"
        );

        if (rfqFilteredData.length > 0) {
          setRfqTotalData(rfqFilteredData.length);
        }
        ...
        const profileUpdateData = result.data.data.filter(
          (item: RfiSupplierInterface) => item.OBJECT_TYPE === "PROFILE_UPDATE"
        );

        if (profileUpdateData.length > 0) {
          // setSUpplierList(filteredData);
          setProfileUpdateTotal(profileUpdateData.length);
        }
