...
export default function RfiRequestForInvoicePage() {
  ...
  //store
  const { setRfiSupplierListlength, setRfiTabNo, rfiTabNo, setStageLevelInStore, setTemplateIdInStore } = useRfiStore();
  //store
  ...
  const navigateTo = (userId: number, rfiId: number, stageLevel: number, tempId: number) => {
    ...
    try {
      ...
      if (result.data.status === 200) {
        ...
        if (newInfoFilteredData.length > 0) {
          console.log("supplier: ", newInfoFilteredData);
          setSUpplierList(newInfoFilteredData);
        ...
        if (invoiceFilteredData.length > 0) {
          ...
          console.log("supplier: ", invoiceFilteredData);
          setSUpplierList(invoiceFilteredData);
        }
                ...
                <div className="overflow-auto max-h-[400px] rounded-lg shadow-lg custom-scrollbar">
                  <table className="min-w-full divide-y divide-gray-200 rounded-lg">
                    ...
                    {supplierList.map((e, i) => (
                      <tbody
                        onClick={() => {
                          navigateTo(e.OBJECT_ID, e.ID, e.STAGE_LEVEL, e.TEMPLATE_ID);
                        }}
                        key={e.ID}
