...
import ShipmentListService from "../../po_supplier/service/ShipmentListService";
// import ShipmentInterface from "../interface/ShipmentInterface";
import ShipmentInterface from "../../po_supplier/interface/ShipmentInterface";
...
export default function BuyerShipmentListPage() {

  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [shipmentList, setShipmentList] = useState<ShipmentInterface[]>([]);

  const { setShipmentIdInStore, setGrnNumberInStore, poHeaderInStore, setPoNumberInStore } = useSupplierPoStore();
  ...
  const navigateTo = (shipmentIdStore: number, grnNumber: string, poNumber: number) => {
    setShipmentIdInStore(shipmentIdStore);
    setGrnNumberInStore(grnNumber);
    setPoNumberInStore(poNumber.toString())
    setPageNo(4);
  }
                ...
                <table className="min-w-full divide-y divide-gray-200 border-[0.2px] border-borderColor rounded-md" style={{ tableLayout: "fixed" }}>
                  ...
                  <tbody>
                    {shipmentList.map((item, index) => (
                      <tr onClick={() => {
                        navigateTo(item.SHIPMENT_ID, item.EBS_GRN_NO, item.PO_NUMBER);
