...
import PoListService from "../../po/service/PoListService";
...
export default function BuyerPoListPage() {
  ...
  const { setPageNo, setSinglePo, setSupplierIdInStore, setHeaderTermFilePath } = useBuyerPoStore();
  //  store dev A PUSH
  ...
  const getPoList = async (ofs: number, lmt: number, ps: string) => {
    try {
      setIsLoading(true);
      const result = await PoListService(token!, ps, null, ofs, lmt); //IN PROCESS
      if (result.data.status === 200) {
        setHeaderTermFilePath(result.data.header_term_file_path);
        setPoList(result.data.data);
        console.log("poList: ", result.data.data);
  ...
  const navigateToDetails = (po: PoInterface) => {
    setSinglePo(po);
    setSupplierIdInStore(po.USER_ID);
    setPageNo(2);
  };
