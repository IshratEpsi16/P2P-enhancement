...
export default function ApprovalPage() {
  ...
  const getApproveHierarchyNewInfo = async () => {
    ...
    try {
      ...
      if (result.data.status === 200) {
        ...
        if (
          decodedToken?.decodedPayload?.NEW_INFO_INITIATOR_STATUS &&
          Object.keys(decodedToken.decodedPayload.NEW_INFO_INITIATOR_STATUS)
            .length > 0
        ) {
          const initiatorStatus = {
            STAGE_LEVEL: 0,
            APPROVER_FULL_NAME:
              decodedToken.decodedPayload.NEW_INFO_INITIATOR_STATUS.FULL_NAME,
            ACTION_CODE:
              decodedToken.decodedPayload.NEW_INFO_INITIATOR_STATUS.ACTION_CODE,
            ACTION_DATE:
              decodedToken.decodedPayload.NEW_INFO_INITIATOR_STATUS.ACTION_DATE,
            ACTION_NOTE:
              decodedToken.decodedPayload.NEW_INFO_INITIATOR_STATUS.NOTE,
            APPROVER_ID: 0,
            APPROVER_USER_NAME:
              decodedToken.decodedPayload.NEW_INFO_INITIATOR_STATUS.USER_NAME,
            EMAIL_ADDRESS: "",
            IS_MUST_APPROVE: 1,
            PROPIC_FILE_NAME: "",
            STAGE_ID: 0,
        ...
        console.log("new hierarchy List: ", updatedHierarchyList);
      } else {
        setHierarchyNewInfoLoading(false);
        // showErrorToast(result.data.message);
      }
  ...
  //     console.log("Decoded Header:", decodedHeader);
  //     console.log("Decoded Payload:", decodedPayload);
  //     console.log("state: ", decodedPayload.PROFILE_UPDATE_UID);
  ...
  //       const submissionStatus = decodedPayload.SUBMISSION_STATUS;
  //       const isRegComplete = decodedPayload.IS_REG_COMPLETE;
  //       const isProfileId = decodedPayload.PROFILE_UPDATE_UID;
  //       // console.log(`userId: ${userId}`);
  //       setProfileUid(isProfileId);
  //       return {
  //         decodedHeader,
  //         decodedPayload,
  //         userId,
