...
let passwordReset = async (updateData, whereData) => {
    ...
    let query = `UPDATE ${dbName}.${user_table} ${updateClause} ${whereClause}`;
...
let getOldPassword = async (user_id)=>{
    let query =`select USER_PASSWORD from ${dbName}.${user_table} where USER_ID =:user_id`;
    return query;
}
