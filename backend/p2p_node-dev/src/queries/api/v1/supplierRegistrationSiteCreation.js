...
let table_name = "XXP2P_SUPPLIER_SITE";
...
BEGIN
    ...
    ELSIF IS_EXIST = 1 THEN
        IF v_status = 'ACTIVE' THEN 
            UPDATE XXP2P.XXP2P_SUPPLIER_SITES_OU 
...
let siteOUByApprover = async (
  ...
  let query = `
          ...
          ELSIF IS_EXIST = 1 THEN 
              DELETE FROM XXP2P.XXP2P_SUPPLIER_SITES_OU
              WHERE USER_ID = v_suppid AND SITE_ID = v_siteid AND ORGANIZATION_ID = v_orgid;
              :MESSAGE := 'Operating Unit Removed Successfully';
              :STATUS := 200;
          END IF;
  ...
  `SELECT 
    ...
    CASE 
      WHEN SSO.USER_ID IS NOT NULL 
           AND SSO.SITE_ID IS NOT NULL 
           AND SSO.ORGANIZATION_ID IS NOT NULL 
...
ON 
    ...
    AND SSO.SITE_ID = NVL(:SITE_ID, SSO.SITE_ID)
JOIN 
    GL_PERIOD_STATUSES_V GLPS 
ON 
    HOU.SET_OF_BOOKS_ID = GLPS.SET_OF_BOOKS_ID
JOIN 
    GL_LEDGERS GL 
ON 
...
let siteOUListWithOutSite = async (USER_ID) => {
  let query = 
  `SELECT 
    HOU.ORGANIZATION_ID,
    HOU.SHORT_CODE,
...
LEFT JOIN 
    GL_PERIOD_STATUSES_V GLPS 
ON 
    GLPS.SET_OF_BOOKS_ID = HOU.SET_OF_BOOKS_ID
WHERE
    US.USER_ID = :USER_ID
...
let addressLineCheck = async (USER_ID, ADDRESS_LINE1) => {
  let query = `
  select 
    ADDRESS_LINE1
FROM
    XXP2P.XXP2P_SUPPLIER_SITE
...
let siteDelete = async (SITE_ID) => {
  let query = `
  DECLARE
  BEGIN
  DELETE FROM XXP2P.XXP2P_SUPPLIER_SITE
  WHERE ID = :SITE_ID;
  COMMIT;
  END;
  `;
