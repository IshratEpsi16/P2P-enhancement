...
const supplierAgentModel = require("../../../../models/api/v1/supplierAgent");
...
let agent_etin_file_path = `${process.env.backend_url}${process.env.agent_etin_file_path_name}`;
...
router.post("/add", [verifyToken], async (req, res) => {
  ...
  finalData.IS_AGENT = [true, 1, "1", "true"].includes(reqData.IS_AGENT)
    ? 1
    : 0;
  ...
  if (!isEmpty(id)) {
    ...
    let existingData = await supplierContactModel.getDataByWhereCondition({
      id: id,
    });
    ...
    existingData = await supplierAgentModel.getDataByWhereCondition({
      SUPPLIER_CONTACT_PERSON_ID: id,
    });

    if (!isEmpty(existingData.rows)) {
      await commonObject.processDbDataToArrayObject({
        queryResult: existingData,
      });
  ...
  finalData.POSITION = reqData.POSITION;
  ...
  // }
  finalData.NID_PASSPORT_NUMBER = reqData.NID_PASSPORT_NUMBER;
  ...
  if (
    ...
    req.files.nid_or_passport_file
  ) {
    let fileUploadCode = {};
  ...
  if (finalData.IS_AGENT) {
    ...
    finalAgentData.CONTACT_PERSON_NAME = req.body.agent_contact_person_name;
    ...
    // if (validateDataCheck.success == false)
    //     return res.status(400).send({ "success": false, "status": 400, "message": validateDataCheck.message });
    finalAgentData.CONTACT_PERSON_POSITION =
      reqAgentData.CONTACT_PERSON_POSITION;
    ...
    finalAgentData.AGENT_ADDRESS = req.body.agent_registered_address;
    ...
    if (!isEmpty(reqAgentData.AGENT_SINCE_DATE)) {
      finalAgentData.AGENT_SINCE_DATE = new Date(reqAgentData.AGENT_SINCE_DATE);
    }
    ...
    //       success: false,
    //       status: 400,
    //       message: "Please give agent mobile number.",
    //     });

    /*let validateMobileNumber = await commonObject.isValidPhoneNumber(
      reqAgentData.BD_PHONE_NUMBER
    );
    if (validateMobileNumber == false)
      return res
        ...
        .send({
          success: false,
          status: 400,
          message: "Agent mobile number is not valid",
        });*/
    ...
    if (!isEmpty(reqAgentData.IRC)) {
      ...
      if (validateId.success == false)
        return res.status(400).send({
          success: false,
          status: 400,
          message: "IRC should be number.",
        });
      finalAgentData.IRC = validateId.data;
    }

    if (!isEmpty(reqAgentData.BD_PERMISSION_NO)) {
      let validateId = await commonObject.checkItsNumber(
        reqAgentData.BD_PERMISSION_NO
      );
      if (validateId.success == false)
        return res.status(400).send({
          success: false,
          status: 400,
          message: "Bd permission no should be number.",
        });
      finalAgentData.BD_PERMISSION_NO = validateId.data;
    }

    finalAgentData.BD_PERMISSION_NO = reqAgentData.BD_PERMISSION_NO;
    ...
    /* if (req.files && Object.keys(req.files).length > 0 && req.files.etin_file) {
      ...
      if (req.files && Object.keys(req.files).length > 0 && req.files.etin_file) {
        ...
        finalData.TIN_FILE_NAME = fileUploadCode.fileName;
        finalData.TIN_FILE_ORIGINAL_NAME = fileUploadCode.fileOriginalName;
    }
  ...
  if (willUpdate) {
    ...
    contact_id = result.outBinds[0][0];
    if (!result || result.rowsAffected == undefined || result.rowsAffected < 1)
      return res.status(500).send({
        success: false,
        status: 500,
  ...
  return res.status(200).send({
    success: true,
    status: 200,
    ...(contact_id !== null && { id: contact_id }),
    message: willUpdate
      ? "Supplier contact info update successfully done"
      : "Supplier contact info successfully add",
  });
});
...
router.post("/details", [verifyToken], async (req, res) => {
  ...
  if (queryResult.finalData[0].IS_AGENT) {
    // get agent info
    let existingAgentData = await supplierAgentModel.getDataByWhereCondition({
      SUPPLIER_CONTACT_PERSON_ID: id,
    });

    if (!isEmpty(existingAgentData.rows)) {
      await commonObject.processDbDataToArrayObject({
        queryResult: existingAgentData,
      });
...
router.post("/contact-ou", verifyToken, async (req, res) => {
  let userId = req.decoded.USER_ID;
  let reqData = {
  ...
  if (isEmpty(reqData.CONTACT_ID)) {
    return res.status(400).send({ message: "Enter Contact ID", status: 400 });
  }
  ...
  let queryResult = await supplierContactModel.contactOU(
    userId,
    reqData.CONTACT_ID,
    reqData.SHORT_CODE,
    reqData.NAME,
...
router.post("/contact-ou-list", verifyToken, async (req, res) => {
  let userId = req.decoded.USER_ID;
  let reqData = {
    CONTACT_ID: req.body.CONTACT_ID,
  };
  ...
  let result = {
    message: `Contact Operating Unit List`,
    status: 200,
    data: [],
  };
  if (isEmpty(reqData.CONTACT_ID)) {
    return res.status(400).send({
      message: "Please Give Contact ID.",
      status: 400,
      success: false,
    });
  }

  let queryResult = await supplierContactModel.contactOUList(
    userId,
    reqData.CONTACT_ID
  );
  console.log(queryResult);
...
router.post("/contact-site-add", verifyToken, async (req, res) => {
  let userId = req.decoded.USER_ID;
  let reqData = {
    CONTACT_ID: req.body.CONTACT_ID,
  ...
  if (isEmpty(reqData.SITE_ID)) {
    return res
      .status(400)
      .send({ message: "Please Select A Site", status: 400, success: false });
  }
  if (isEmpty(reqData.CONTACT_ID)) {
    return res.status(400).send({
      message: "Please Give Contact ID.",
  ...
  try {
    let queryResult = await supplierContactModel.contactSiteMapping(finalData);
    console.log(queryResult);

    if (!isEmpty(queryResult.outBinds[0][0])) {
      value.message = "Site Added Successfully.";
      value.status = 200;
      return res.status(value.status).send(value);
    } else {
      value.message = "Site Not Added!.";
      value.status = 500;
      return res.status(value.status).send(value);
    }
  } catch (error) {
    console.error(error);
    value.message = `An error occurred while adding the site.`;
    value.status = 500;
    return res.status(value.status).send(value);
  }
});

router.post("/contact-site-list", verifyToken, async (req, res) => {
  let userId = req.decoded.USER_ID;
  let reqData = {
    CONTACT_ID: req.body.CONTACT_ID,
  };
  ...
  try {
    let queryResult = await supplierContactModel.contactSiteMappingList(
      reqData.CONTACT_ID
    );
    await commonObject.makeArrayObject(queryResult);
    value.data = queryResult.queryResult.finalData;
    value.message = "Site List";
    value.status = 200;
    return res.status(value.status).send(value);
  } catch (error) {
    console.error(error);
    value.message = `An error occurred while getting the site list.`;
    value.status = 500;
    return res.status(value.status).send(value);
  }
});
