...
router.post(`/ou-list`, verifyToken, async (req, res) => {
  try {
    ...
    // OU List
    let queryResult = await ouModel.ouListByID(reqData.ORG_ID);
...
router.post(`/le-details-by-org-id`, verifyToken, async (req, res) => {
  try {
    //userId = req.user ? req.user.USER_ID : null;
    let finalData = {};
    let value = {
