...
const queries = require("../../../queries/api/v1/supplierRegistrationSiteCreation");
...
let siteOUByApprover = async (
  ...
  try {
    let query = await queries.siteOUByApprover();
    let result = await connectionP2pORACLE.execute(query, {
      SUPPLIER_ID,
      SITE_ID,
      SHORT_CODE,
...
let siteOUListWithOutSite = async (USER_ID) => {
  ...
  try {
    let query = await queries.siteOUListWithOutSite();
    let result = await connectionP2pORACLE.execute(query, {
      USER_ID
    });
...
let addressLineCheck = async (USER_ID, ADDRESS_LINE1) => {
  ...
  try {
    let query = await queries.addressLineCheck();
    let result = await connectionP2pORACLE.execute(query, {
      USER_ID: USER_ID,
...
let siteDelete = async (SITE_ID) => {
  ...
  try {
    let query = await queries.siteDelete();
    let result = await connectionP2pORACLE.execute(query, {
      SITE_ID: SITE_ID,
    });
