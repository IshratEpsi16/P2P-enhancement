...
const queries = require('../../../../queries/api/v1/supplierApproval/supplierAgent');
...
let addNew = async (info, conn = undefined) => {
    ...
    try {
        ...
        let result = await connection.execute(query, queryValue, { autoCommit: true });
...
let deleteById = async (id = 0, conn = undefined) => {
    ...
    try {
        ...
        let query = await queries.deleteById();
        let result = await connection.execute(query, [id], { autoCommit: true });
